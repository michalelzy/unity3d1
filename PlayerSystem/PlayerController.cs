...
// 把模板Scene拖入Unity中，就自动把场景包括所有的角色还有控制代码都放入场景中了。点播放就可以玩
// 场景是一个Unity正方体图标，是和其他线框图标还有带颜色的图标是有区别的

// Polygon对象中，就是把所有的场景中的地形、障碍都都放进去了
// Directional Light是光线对象
// Player 就是我们要的角色对象了。其中就把控制脚本挂在上去了。但是，没有直接挂载到Player对象中，
// 而是把脚本挂在到了Player子对象 Player_Object 中。
// Player_Object: 
// 点击Player_Object，就可在Inspector面板中对该对象进行各种设置
// 本脚本文件就是挂在到 Player_Object上，所以当在其他场景中添加这个角色的时候，也是可以直接操作的。


// 定义命名空间
namespace ElmanGameDevTools.PlayerSystem
{
    // 让PlayerController这个类，出现在“Elman Game Dev Tools/Player System/Player Controller"
    // 这个路径中。什么意思呢？就是我们在 add Component的时候，可以搜索，也可以从路径中去一步一步打开然后
    // 添加。而这个AddComponentMenu特性（俗称Attributes）加上之后，就可以通过路径去找了，而不是只能
    // 通过搜索才能找到这个类PlayerController
    [AddComponentMenu("Elman Game Dev Tools/Player System/Player Controller")]
    public class PlayerController : MonoBehaviour
    {
        #region Variables 
        // 在Inspector中添加一个分组标题"REFERENCES"。通过这些操作，可以在开发时可以在Inspector
        // 去操作，而不需非得改代码
        [Header("REFERENCES")]
        // 鼠标悬停时的提示文字
        [Tooltip("Reference to the Character Controller component")]
        // public 让Inspector可以进行可视化操作；可控制的角色对象
        ...
        // C#中，double类型是8字节；浮点数float是4字节，性能更好，那么在写数字时要加一个f或者F
        ...
        // 是Unity的一个特性，可以在 Inspector 中限定为一个滑动模块，有上限和下限
